...
def create_pairs(x, digit_indices):
    ...
    for d in range(num_classes):
        for i in range(n):
            ...
            if d == 5 and dn == 7:  # shoes
                labels += [1, 0.75]
            elif (d == 0 and dn == 2) or (d == 0 and dn == 4) or (d == 2 and dn == 4):  # tshirt, pullover, coat
                labels += [1, 0.75]
            elif (d == 0 and dn == 6) or (d == 2 and dn == 6) or (d == 4 and dn == 6):  # shirt with tshirt, pullover, coat
                labels += [1, 0.75]
            elif (d == 5 and dn == 9) or (d == 7 and dn == 9):  # ankle boot with sandal and sneaker
                labels += [1, 0.25]
            else:
                labels += [1, 0]
...
def compute_accuracy(y_true, y_pred):
    ...
    elif 0.25 <= y_true.any() < 0.75:
        pred = (0.25 <= y_pred.ravel() < 0.5)
        return np.mean(pred == 0.25)

    elif 0.75 <= y_true.any() < 1:
        pred = (0.5 <= y_pred.ravel() <= 0.75)
        return np.mean(pred == 0.75)

    elif y_true.any() == 1:
        pred = (y_pred.ravel() > 0.75)
